...
const { isCompound } = require("./types/JSONSchema");
 ...
 * types). The spec leaves it up to implementations to decide what to do with this loosely-defined behavior.
 */
module.exports.typesOfSchema = (schema) => {
  ...
  for (const [schemaType, f] of Object.entries(matchers)) {
    if (f(schema)) matchedTypes.push(schemaType);
  }
...
const matchers = {
  ...
  CUSTOM_TYPE() {
    return false;
  }, // Explicitly handled before we try to match
  NAMED_ENUM(schema) {
  ...
  UNNAMED_ENUM(schema) {
    if ("tsEnumNames" in schema) return false;
    if (
      schema.type &&
      schema.type !== "boolean" &&
      schema.type !== "integer" &&
  ...
  UNNAMED_SCHEMA() {
    return false;
  }, // Explicitly handled as the default case
