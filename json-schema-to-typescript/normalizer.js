...
rules.set("Empty Object to Record", (schema) => {
  if (
    schema.type === "object" &&
...
rules.set("Add empty `required` property if none is defined", (schema) => {
  if (isObjectType(schema) && !("required" in schema)) schema.required = [];
});

rules.set("Transform `required`=false to `required`=[]", (schema) => {
  if (schema.required === false) schema.required = [];
});
...
rules.set("Transform format -> comment", (schema) => {
  if (schema.format) {
    schema.description = appendToDescription(
      schema.description,
      `@format ${schema.format}`
    );
    // schema.comment = `${schema.comment ? (`${schema.comment}\n`) : ''} @format ${schema.format} `
  }
});

rules.set("format binary -> Blob", (schema) => {
  if (schema?.format == "binary") {
    schema.oneOf = [{ type: schema.type }, { $ref: "Blob" }];
    delete schema.type;
  }
});
...
rules.set("Normalize schema.minItems", (schema, _fileName, options) => {
  if (options.ignoreMinAndMaxItems) return;
...
rules.set(
  ...
  (schema, _fileName, options) => {
    if (options.ignoreMinAndMaxItems || options.maxItems === -1) return;
    if (!isArrayType(schema)) return;
...
rules.set("Remove extends, if it is empty", (schema) => {
  if (!schema.hasOwnProperty("extends")) return;
  if (
    schema.extends == null ||
...
rules.set("Make extends always an array, if it is defined", (schema) => {
  ...
  if (!Array.isArray(schema.extends)) schema.extends = [schema.extends];
});
...
rules.set("Transform nullable -> anyOf", (schema) => {
  if (schema.nullable === true) {
    ...
    delete schema.allOf;
    delete schema.enum;
    schema.oneOf = [copiedSchema, { type: "null" }];
  }
});
